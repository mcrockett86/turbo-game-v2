...
│       ├── types.ts            # TypeScript interfaces (Dog, Zone, Room, etc.) — ZoneType is 'fp' | 'tp' (search type was cut 2026-08-24)
...
- [x] **Real audio** — ✅ COMPLETED 2026-08-24: `engine/music.ts` (deterministic per-zone beds: bass/arp/pad/melody for all 17 tracks, grouped by mood) + rewritten `engine/audio.ts` (mix bus sfx→master / music→duck→master, lookahead scheduler, ref-counted ducking, layered SFX: bark/select/pickup/footstep/threat_start/success/fail, bounded voice GC). main.ts ducks the bed during threat minigames + companion dialogue, plays threat start/success/fail. Perf gate: post-audio p95 +0.1ms vs pre (limit +50ms) — see `perf/baseline-post.json` + `npm run perf:verify:post`.
- [x] **Unit test layer** — ✅ COMPLETED 2026-08-23: vitest@2 + jsdom, 67 tests in `tests/unit/` (~1.5s) covering `state.ts` (27), `transitions.ts` (8), `threats.ts` (10), `data.ts` integrity (19). `npm test` runs the suite; Playwright config excludes `tests/unit/`. The new `data.ts` cross-reference test caught 7 unreachable threats (see note below) which are now wired. See `tests/unit/*.test.ts`.
- [x] **`search` zone type** — ✅ RESOLVED 2026-08-24 (cut): removed `search` from `ZoneType`/`RendererType` in `types.ts` and the dead FP fallback branch in `main.ts`. No zone in `data.ts` used it; a new `data.test.ts` assertion guards against any future zone declaring an unknown type.
- [x] **Long-session stability pass** — ✅ COMPLETED 2026-08-24: `tests/stability.spec.ts` drives zone-swap churn + 20 threat cycles + an idle soak and asserts window-listener count stays bounded (no per-zone/per-threat leak), audio live-voice list stays bounded (scheduler GC works, and the bed actually produces voices), the frame loop keeps advancing, and there are no unhandled exceptions. Fixed two real leaks found by the pass: (1) `ThreatManager.start()` re-armed key listeners on every call → now dedupes; (2) a stale zone-entry-threat `setTimeout` could fire after navigating away → `enterZone` now guards on zone + screen + not-busy. See `tests/stability.spec.ts`.
- [x] **Content & companion coverage** — ✅ COMPLETED 2026-08-24: all 15 companions are now reachable — the 7 missing ones (Luna, Whiskers, Max, Coco, Blossom, Bookworm, Baker) are wired as TP NPCs (dog_park) or FP `dog_friend` features with an explicit `companion` field (new on `RoomFeature`; `handleFeature` prefers it over the `zone.companions` fallback). Item coverage: all 69 items now attachable — 48 previously-orphaned items wired as thematically-fitting `treasure` features across zones, every category (comfort/clue/key/food/utility/story/crafting/quest/rare) has ≥ 1 reachable item. `tests/unit/coverage.test.ts` (6 tests) enforces all of this, and now counts FP room features (not just zone-level ones) so it mirrors what the renderers actually expose.
- [x] **Build & perf budget** — ✅ COMPLETED 2026-08-24: `engine/perf.ts` ring-buffer (avg/p50/p95/max/dropped), budgets locked (p50<20ms, p95<50ms, ≤10 dropped/10s). `scripts/perf-check.ts` navigates all 18 zones headlessly and records/verifies `perf/baseline-pre.json` (p50 16.7ms, p95 16.7ms, 1 dropped — recorded **before** audio). `tests/unit/perf-budget.test.ts` gates the pre→post-audio drift so Sprint 6 item 1 can't regress perf without re-baselining. `npm run perf:baseline | perf:verify | perf:verify:post`.

> **Threat-coverage note (2026-08-23):** the new `data.ts` unit test asserts every one of the 40 threats is actually reachable. It caught 7 threats that were defined but referenced by no zone/feature/legacy-map (construction, squirrel, neighbor_dog, pet_shop_cat, fog, lightning, garden_insect) — they are now wired as interactable features in thematically-fitting zones, so all 40 are genuinely triggerable.
